...
async def get_producer_by_id(id: str = Path(..., min_length=24, max_length=24),
    ...
    if fields is not None:
        ...
        producer = producer_collection.find_one(
            {"_id": ObjectId(id)}, fields_to_return)
    else:
...
async def get_producer_by_phone_number(phone_number_param: str = Path(..., min_length=10, max_length=15, regex="[0-9]{10,15}"),
    ...
    if fields is not None:
        fields_to_return = {key: 1 for key in fields}
        producer = producer_collection.find_one(
            {"phone_number": phone_number_param}, fields_to_return)
    else:
        producer = producer_collection.find_one(
...
async def post_producer(producer: producer_post):
    ...
    producer_document = producer_collection.find_one(
        {"phone_number": producer.dict()["phone_number"]})
    if producer_document:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Proudcer already exists with phone number, " +
            producer_document["phone_number"]
        )
    ...
    inserted_producer = producer_collection.find_one(
...
async def delete_producer_menu_items(*, id: str = Path(..., min_length=24, max_length=24), day: day, meal_type: meal_type, menu_id: str):
    ...
    if ObjectId(menu_id) not in producer["menu"][day][meal_type]:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Food item with id of " + menu_id +
            " not found in " + day + "'s " + meal_type + " array"
        )
...
async def delete_producer_by_id(id: str):
    ...
    deactive_producer = deactivated_producer_collection.insert_one(
        active_producer)
