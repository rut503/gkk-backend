...
from app.schemas.review_for_consumer_schema import (
    review_for_consumer_serializer,
    reviews_for_consumer_serializer,
)
...
@review_for_consumer_router.get("/testing")
async def get_test():
    return {"message": "hi"}
...
async def get_all_review_for_consumer_by_user(
    ...
    if consumer_id is not None and producer_id is not None:
        ...
        review_document_cursor = review_for_consumer_collection.find(
            {"consumer_id": ObjectId(consumer_id), "producer_id": ObjectId(producer_id)}
        )
...
async def get_review_for_consumer_by_id(
    ...
    review_document_returned = review_for_consumer_collection.find_one(
        {"_id": ObjectId(id)}
    )
    ...
    serialized_review_document = review_for_consumer_serializer(
        review_document_returned
    )
    return serialized_review_document


# Post a single review
@review_for_consumer_router.post(
    "/",
    response_model=review_for_consumer_response,
...
async def post_review_for_consumer(review: review_for_consumer_post):
    ...
    if not result.inserted_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Error while inserting"
        )

    # Retrieving newly added document
    inserted_review = review_for_consumer_collection.find_one(
...
async def update_review_for_consumer(
    *,
    id: str = Path(..., min_length=24, max_length=24),
    review: review_for_consumer_put
):
    review_to_update_dict = review_for_consumer_serializer(
        review_for_consumer_collection.find_one({"_id": ObjectId(id)})
    )
    ...
    result = review_for_consumer_collection.update_one(
        {"_id": ObjectId(review_identifier)}, {"$set": updated_model.dict()}
    )

    if result.modified_count != 1:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Error while updating"
        )

    updated_review = review_for_consumer_collection.find_one(
